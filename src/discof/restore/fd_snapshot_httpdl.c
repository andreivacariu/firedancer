...
fd_snapshot_httpdl_init_connection( fd_snapshot_httpdl_t * self ) {
  ...
  self->req_deadline = fd_log_wallclock() + (long)FD_SNAPSHOT_REQUEST_TIMEOUT;
  ...
  self->state = FD_SNAPSHOT_HTTP_STATE_REQ;
  return 0;
}
...
fd_snapshot_httpdl_req( fd_snapshot_httpdl_t * self ) {
  ...
  if( self->req_tail == self->req_head ) {
    self->state = FD_SNAPSHOT_HTTP_STATE_RESP;
  }
...
fd_snapshot_httpdl_resp( fd_snapshot_httpdl_t * self ) {
  ...
  ulong   bufsz     = FD_SNAPSHOT_HTTP_RESP_BUF_MAX - self->resp_head;
  assert( self->resp_head <= FD_SNAPSHOT_HTTP_RESP_BUF_MAX );
  ...
  self->state = FD_SNAPSHOT_HTTP_STATE_DL;
  return 0;
}
