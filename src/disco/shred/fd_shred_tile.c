          ...
          if( FD_LIKELY( entry_meta->parent_block_id_valid ) ) {
            if( entry_meta->parent_offset==1 && ctx->slot%4!=0 ) {
              if( !fd_memeq( ctx->chained_merkle_root, entry_meta->parent_block_id, FD_SHRED_MERKLE_ROOT_SZ ) ) {
                FD_LOG_WARNING(( "chained_merkle_root!=block_id (block_id=....,reset_slot=%lu,parent_slot=%lu) - ignored", ctx->slot, ctx->slot-entry_meta->parent_offset ));
              }
            }
            ...
            FD_LOG_WARNING(( "invalid_block_id_cnt++ (block_id=null,reset_slot=%lu,parent_slot=%lu) - ignored", ctx->slot, ctx->slot-entry_meta->parent_offset ));
